...
function sourcesMatchManifest(currentSources, candidateManifest) {
  ...
  const savedNames = Object.keys(savedSources);

  if (currentNames.length !== savedNames.length) return false;

  for (const fileName of currentNames) {
...
function normaliseSearchQueries(queries) {
...
async function searchIndex(queries, topK) {
  const queryList = normaliseSearchQueries(queries);
  const queryTerms = queryList
    .map(query => ({ query, terms: getQueryTerms(query) }))
      ...
      for await (const line of reader) {
        ...
        for (const entry of queryTerms) {
          ...
          for (const term of entry.terms) {
            queryScore += countOccurrences(text, term);
          }

          if (queryScore > 0) {
            matchedQueries.push(entry.query);
...
function findChunkOverlap(previousContent, currentContent) {
  const previous = String(previousContent || "");
  const current = String(currentContent || "");
  const maxOverlap = Math.min(previous.length, current.length, 600);
...
async function readDocument(file, offset = 0, limit = 40000) {
  ...
  const requestedEnd = safeOffset + safeLimit;
  ...
  const capturePiece = piece => {
    ...
    const pieceStart = totalCharacters;
    const pieceEnd = pieceStart + piece.length;

    if (pieceEnd > safeOffset && pieceStart < requestedEnd) {
      const from = Math.max(safeOffset - pieceStart, 0);
      const to = Math.min(requestedEnd - pieceStart, piece.length);
      if (to > from) pieces.push(piece.slice(from, to));
    }

    totalCharacters = pieceEnd;
  };
  ...
  for (const shard of activeManifest.shards || []) {
    ...
    try {
      for await (const line of reader) {
        ...
        if (!previousContent) {
          capturePiece(currentContent);
        } else {
          const overlap = findChunkOverlap(previousContent, currentContent);
          let piece = currentContent.slice(overlap);
  ...
  return {
    ...
    totalCharacters,
    chunkCount,
    hasMore: nextOffset < totalCharacters,
    nextOffset: nextOffset < totalCharacters ? nextOffset : null,
    text
  };
}
