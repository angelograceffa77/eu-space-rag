...
function scoreItemAgainstQuery(item, query) {
  ...
  for (const term of terms) {
    ...
    const totalHits = contentHits + pathHits;
    ...
    const weight = distinctive ? 4 : 1;

    if (distinctive) distinctiveMatched += 1;

    // Content matters, but filename/folder matches are particularly useful
    // for country-, institution- and document-specific retrieval.
    score += contentHits * weight;
    score += pathHits * weight * 12;
  }

  if (matchedTerms === 0) return null;

  // Reward chunks that cover several concepts from the same query rather
  // than repeating one generic word many times.
  score += matchedTerms * matchedTerms * 3;

  // A distinctive term such as Italia / Italy should strongly outrank a
  // document matching only generic words such as space / law / operator.
  score += distinctiveMatched * 18;

  // Reward an exact multi-word phrase when present.
  if (phrase.length > 6) {
    if (contentText.includes(phrase)) score += 30;
    if (pathText.includes(phrase)) score += 80;
  }
